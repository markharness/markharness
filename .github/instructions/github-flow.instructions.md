...
Post-merge cleanup may be performed in a separate session. First update the local `main` from its remote, then verify that the recorded pull request is merged on GitHub; do not rely only on Git ancestry because squash and rebase merges may replace commit identities. Do not treat a branch-name-only pull-request search as conclusive: a local cleanup branch may have a different name from the pull request's original head branch. Resolve the candidate worktree's HEAD commit and query GitHub for pull requests associated with that commit, then corroborate the result with the pull request's merged state and the updated `main` history.

Before removing anything, verify that the candidate path is the recorded task worktree, it is not the current worktree, and inspect both tracked and untracked changes with `git status --short`. For every remaining file, inspect its content and determine whether it is already committed, incorporated into the merged pull request, or intentionally transcribed into a durable issue or document. Report any unique information that deletion would lose; do not equate a summarized issue with a complete transcription. Present the verified PR, branch, absolute worktree path, remaining files, and preservation assessment to the user and obtain explicit authorization to delete them. If the PR is not confirmed merged, the identity of the worktree is unclear, or the disposition of any remaining file is unresolved, do not remove it; report the condition instead. Never use forced worktree or branch deletion to bypass these checks.
