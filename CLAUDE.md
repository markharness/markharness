...
- **実装時のレビュー**
  1. 実装が完了したら、タスクを一度終了（Done）させて自動的に「Codexのレビューゲート」を発火させてください。
  2. もしCodexの判定が「BLOCKED（問題あり）」になった場合、私（人間）に許可を求めず、指摘されたバグや改善点を読み取って即座に修正コードを自律的に実装してください。
  3. 修正後、再度タスクを終了させてCodexのレビューを受けてください。
  4. Codexのレビューが「PASSED（問題なし）」になるか、または最大3回ループを繰り返すまで、この「修正➔レビュー」のサイクルをあなたの判断で自動で回し続けてください。
  5. ただしBLOCKEDの理由が実際のコード指摘ではなく、利用制限・タイムアウト・空応答などCodexタスクそのものが実行できなかったことによるもの(既知の上流バグ: [openai/codex-plugin-cc#248](https://github.com/openai/codex-plugin-cc/issues/248))の場合は、修正ループに入らず直ちにレビューゲートを無効化し、ユーザーに状況を報告してください。
...
- **情報の配置とコメントの責務** — Codeは現在のWhat/How、コメントは局所的なWhy・不変条件・コードから明らかでない制約、ADRは設計判断と代替案、Git履歴は変更の経緯を記録する。コメントを書く前に、コード・型・テスト・命名で表現できないか確認し、実装の逐語説明や設計全体の理由・変更履歴はコメントに書かない。設計レベルの理由はADRへ、変更の経緯はGit履歴へ置く。詳細: [CONTRIBUTING.md](./CONTRIBUTING.md#information-placement-and-comments)
- **日本語版/英語版ドキュメントの同時更新** — `docs/ja/`と`docs/en/`は内容を鏡合わせで維持する運用。`docs/`配下(論文・ADR・design仕様書を含む)を編集する際は、同じ変更を両言語のファイルに同一PR内で反映する。どちらかのみを更新して他方を放置しない。
  - **例外: AI実行用の単一ファイル文書。** `docs/knowledge-from-code.ai.md` のように、`docs/ja/`・`docs/en/` に属さず `docs/` 直下に置かれ、「チャットで使われている言語で応答する」ことをAIに指示する形式の文書は、この鏡合わせ運用の対象外とする。この種の文書はプロンプトとして1ファイルで完結させることが目的であり、応答言語はチャット利用者の言語に追従するため、ja/en 分割・同時更新は不要。過去に `docs/ja/knowledge-from-code.standalone.md`・`docs/en/knowledge-from-code.standalone.md` という ja/en 対で運用していたが、`docs/knowledge-from-code.ai.md` への一本化に伴い両方削除した(2026-09-12)。
