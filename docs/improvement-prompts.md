...
**進捗メモ**：本ファイルの一部項目は、当時の指摘後に既に実装・論文反映が完了している。着手前に必ず現状を確認すること（各項目冒頭の状況注記を参照）。項目1・2・3・4・5は完了、項目6は未確認。
...
項目1〜6のうち、実質的に残っているのは項目6のみ（1・2・3・4・5は完了）。項目6は「上記1〜5の改善が実施され次第」が条件のため、今回の項目2・3の完了内容を反映するタイミングで着手する。項目7〜9はいずれもスキーマへの加算的な変更で相互依存はないため、優先度順(7→8→9)に着手するか、項目6と並行で進めても支障ない。ただし各項目末尾に「実装前に私に確認してください」とある設計判断(フィールドの意味論・省略時の扱い)は、着手前に必ず確認を取ること。
