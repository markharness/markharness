...
**進捗メモ**：本ファイルの一部項目は、当時の指摘後に既に実装・論文反映が完了している。着手前に必ず現状を確認すること（各項目冒頭の状況注記を参照）。項目1〜9すべて完了。
...
**状況(2026-08時点)**：完了。`mh-sample-test-case`に`markharness-lineage-scenario-feature`ブランチを作成し、`main`側と異なるファイルへの変更を加えて`--no-ff`マージ、`test4`タグを付与した。`markharness changes lineage --commit`・`changes compute test3 test4`を実際に実行し、結果を`docs/gap-analysis-mh-sample-test-case.md`§8に記録した(`test1`〜`test3`の既存データは変更していない)。なお、着手前の確認で`docs/gap-analysis-mh-sample-test-case.md`旧§8が実際には実行されていない記録(実クローンにタグ・ブランチが存在せず、`.git/index.lock`が空のまま残置)だったことが判明したため、その旧記録は破棄し本項目で改めて実施した。
...
**状況(2026-08時点)**：完了(前提が変化)。本項目の前提だった「V1(markharness側)とV2(`mh-sample-test-case/docs/...V2.md`)の2ファイル」という構図は既に解消されていた：2026-08-10のコミット`cfa5a93`でmarkharness側のファイル名から`V2`サフィックスが削除され(`統合版V2.md`→`統合版.md`)、本文表題と一致する単一の正本になっている。また`mh-sample-test-case/docs/`は`.gitignore`対象かつ現状空で、`...V2.md`は存在しなかった(過去に存在して失われた形跡もなし)。したがって「2ファイルの差分を統合する」作業は不要と判断し、代わりに本項目の実質的な狙い(変更履歴の運用開始・無意味なバージョン番号の積み上げ防止)を単一の正本に対して実施した：論文末尾に「変更履歴(Changelog)」節を新設し、`CLAUDE.md`に運用ルール(実質的な変更がなければバージョン番号もChangelogも増やさない)を追記した。
...
**状況(2026-08時点)**：完了。`ChangeEvent`に`related_events: Vec<String>`(`#[serde(default)]`)を追加し、`markharness changes annotate --related <event_id>`(複数指定可)で追記できるようにした(`src/changes.rs`・`src/cli.rs`)。存在しない`event_id`を指定した場合は`AnnotateError::NotFound`(該当id付き)でエラーになる。論文§3.5に製品化提案として追記した。第三者レビュー(`checklist-changes-lineage-generalization.md`参照)で「`--type`が必須のため`--related`単独で使えない」との指摘があり、`--type`を`Option`化し`--type`/`--related`のいずれか一方の指定を必須とする形に修正済み。
...
**状況(2026-08時点)**：完了。`schema/expected_result.schema.json`に`generated_by`(enum: `manual`/`llm`/`auto_combination`)・`verified_by`(`{ human_review: boolean }`、`human_review`は指定時必須)を追加した(`additionalProperties: false`維持、両方とも`required`には含めない)。`generated_by`省略時は「不明」であり「manual」を意味しない旨をschemaの`description`に明記した。`src/knowledge.rs`の`ExpectedResult`構造体にも対応するフィールドを追加。`prompt_version`・`model`名・`confidence_score`は追加していない。将来のCIゲート(未実装)は論文§7 Future Workに1行追記した。
...
項目1〜9すべて完了。以降、本ファイルへの新規追加が必要になった場合は同じ形式(状況注記→プロンプト本文)で追記すること。
