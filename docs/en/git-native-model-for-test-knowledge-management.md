...
1. **Initial proposal (Proposal A)**: An information model that integrates functional structure, test cases, execution results, and milestones. As a research theme, this overlaps with requirements-test-implementation traceability research (Cleland-Huang, 2011, etc.) and has weak novelty, but "tracking derivation relationships with the version axis as a first-class concept" remained as a differentiation opportunity not found in existing TMSs (Appendix A, Chapter 1 of the deliberation summary).
...
4. **Partial pivot and staged design revisions**: Building on the human-facing model as a foundation, the LLM angle was carved out as future work, and the research theme was narrowed to "Proposal 1: structural representation" alone (Chapter 4 of the deliberation summary). After more than ten rounds of subsequent technical feedback, the following was settled:
   ...
   - Tasks were stratified into "shallow changes that the existing workflow can also handle" and "deep changes that the existing workflow cannot in principle handle," with accuracy (especially in the deep-change stratum) adopted as the primary metric (Section 5.3).
...
The current workflow (a combination of TestRail/Jira/git search) cannot, in principle, answer the question "which of today's tests are affected by a past change," in any of its forms, because it does not hold derivation relationships that span multiple Features as a cross-cutting query target (there are products, such as TestRail, that offer history-comparison/restore functionality for an individual test case, but this is confined to the edit history of an individual test case, and is not a feature that cross-cuttingly identifies the set of test cases affected by a Feature's change; see Sections 1.3 and 2.4). This research fills that gap with a milestone-boundary ChangeEvent model built on top of Git's own object model (Chapter 3). This foundation is not a simple path-based `git diff`/`git log --follow`, but a core design mechanism that combines path-independent ID resolution with directory-level tree-SHA comparison, and it is robust against renames and relocations (Sections 1.3 and 3.3).
...
1. **Core design mechanism**: The design of a model (Chapter 3) that, combined with ancestor search over the commit graph (`git merge-base`), derives version history at milestone boundaries independent of branching workflow, via three elements: (a) path-independent ID resolution that reads a Feature's identifier from the `id:` field of `feature.yml` rather than from a directory path; (b) using, as the unit of comparison, the tree SHA of the entire Feature directory rather than the blob SHA of `feature.yml` alone, so that changes to Condition/ExpectedResult alone are also detected without going through feature.yml; and (c) a content-addressed ID-resolution cache (following the same design philosophy as Git's `commit-graph` auxiliary cache) that composes the tree SHA of `knowledge/` together with the versions of the normalization rules, schema, and tooling. However, the claim of "independence from branching workflow" applies only to the primary lineage derived from the final tree diff (`changes compute`); the lineage audit for merges using ancestor search (`changes lineage`, `true_divergences`) is a secondary feature that presupposes the retention of merge commits, and does not function under squash/rebase workflows (Section 3.4, Table 2). Whereas path-based history tracking (`git diff`, `git log --follow`, etc.) is weak against directory renames and relocations and does not guarantee the logical identity of tree-structured test knowledge, this model avoids that limitation through mechanism (a).
2. A design pattern (Section 3.5) that decouples cross-cutting viewpoints (Axis) from the physical directory structure, expressing many-to-many relationships while preserving the tree structure.
...
**Differentiation of this study**: This is a local/Git-native workflow that integrates Git's commit semantics, structured test knowledge, milestone-boundary ChangeEvents, and re-verification tracking. Unlike the individual-test-case history-comparison/restore functionality provided by existing TMSs (TestRail, etc.), the central differentiator of this model is the ability to cross-cuttingly query version history spanning multiple Features (Section 2.4).
...
### 2.4 Comparison with Existing Test Management Tools and Git-Native Workflows
...
**(1) Commercial TMS / self-hosted TMS**: Major products such as TestRail, Zephyr Scale, Xray, and qTest offer milestone functionality and traceability functionality. TestRail (Enterprise edition) provides a version-history-comparison/restore feature for an individual test case (Test case versioning), but this is confined to the edit history of an individual test case; it does not provide a local/Git-native workflow that integrates Git's commit semantics, structured test knowledge, milestone-boundary ChangeEvents, and re-verification tracking across multiple Features (see References). Across the other major products as well, functionality that treats derivation relationships spanning multiple generations as a cross-cutting query target is generally absent (as of 2026, TestRail has discontinued its on-premises edition and is cloud-only; Xray can be self-hosted via Jira Data Center). Free, self-hostable existing options (Kiwi TCMS, TestLink, Klaros Test Management) have the same limitation.
...
**(3) Structured-metadata-plus-Git-managed tools (GTM, tmt/fmf)**: GTM (the Git Test Management System) advertises the same keyword as this study, "Git-native test management," and is a tool that manages test cases in Markdown on Git. Version management is a manual integer scheme (an optional feature) via a v1/v2/v3 suffix on the filename and a version field, and change history depends on manual CHANGELOG.md entries and Git branch/PR review. No automatic mechanism equivalent to ChangeEvent derivation could be confirmed within the scope of the public documentation (see References). tmt/fmf (an OSS test-execution framework from Red Hat, adopting fmf as its metadata format) is well recognized in both academia and practice. Its `adjust` attribute allows dynamic modification of test metadata according to context such as product, distribution, and architecture, but this is a mechanism for handling spatial variation across environments; no functionality equivalent to version history, change history, release/milestone concepts, or change-impact analysis could be confirmed within the scope of the Core specification (see References). Both conclusions are limited to what could be confirmed from public documentation; tmt's Plans/Stories/Policy specifications and plugin ecosystem, and GTM's source-code implementation, have not been verified.

In practice, rather than using any single one of these tools, it is common to operate a combination of a TMS, an issue-tracking tool (Jira, etc.), and git search — but no such combination allows systematic tracing of changes from past generations. This study is positioned to fill this structural gap, and the evaluation design (Chapter 5) reflects this reality.
...
|---|---|---|---|---|---|
| Commercial TMS (TestRail, etc.) | DB (non-Git) | Undisclosed (note 1) | Individual case history comparison/restore only (no cross-cutting query) | None | Test-case management (snapshot-centric) |
...
| tmt/fmf | YAML (Git-managed, fmf inheritance) | Not applicable (has no concept of version at all) | None | None (`adjust` is spatial branching across environments, orthogonal to the time axis) | Execution portability across multiple environments and CI/CD systems |
...
`FEATURE` does not have a field (a `version` integer) that a human manually manages as a version number. The identifier used for lineage computation is not a value written into front matter, but an identifier that **the Git object store already holds**; `label` is display-only (it is not used in lineage computation). Because this identifier resolution uses the `id:` field of `feature.yml` as the canonical source rather than the directory name or path (Section 3.3), it can, unlike a simple path-based `git log --follow`, continue to track the same Feature after a directory rename or relocation.
...
**(A) The structural generation graph (static, version-independent)**: The `generates` relationship `FEATURE`/`CONDITION` → `TESTCASE`. This is a static structure expressing which test cases are generated from the current Feature/Condition, and it requires no version history. When a developer, on a working branch, wants to know "which TestCases are regenerated by this change I'm making right now," all that is needed is this generation graph plus the current change content (a simple diff between HEAD and a baseline point) — which is, in effect, a scoped `git diff` operation. **This feature is a practical convenience feature and is not included among the study's core contributions or the RQ1 evaluation target** (it has practical value in that existing workflows lack this feature altogether, but it is separated out as an evaluation axis because it does not use the version-history ChangeEvent model at all).

**(B) The version-history ChangeEvent model (derived_from, finalized at milestone boundaries)**: The core model of this study, which expresses how the same Feature has changed across consecutive milestones as a `from_tree_sha`/`to_tree_sha` comparison on `ChangeEvent`. It is a model computed independently for each milestone interval, and is not held as a persistent graph structure with version nodes and edges (for extension to a persistent graph, see Chapter 7, Future Work). Neither existing TMSs nor naive Git-based workflows have this feature, and this is the sole target that RQ1 verifies.

The derivation of the version-history ChangeEvent model (B) proceeds as follows.

- **What the tree SHA provides**: A collision-free identification of Feature-directory contents. If the content differs, the value necessarily differs, so numeric collisions across branch divergence (which can occur when a human manually increments an integer) do not occur. However, this alone tells us nothing about the parent-child relationship of "which tree derived from which tree."
...
### 3.3 ID Resolution: A Non-Committed Cache, and Its Cache Key and Invalidation Conditions
...
**Implementation status**: The CLI implementation's `.markharness-cache/<ref>.json` implements the content-addressed cache key described in this section (a composite of `tree_sha(knowledge/)` + `canonicalization_rule_version` + `id_index_schema_version` + `tool_version`) (`CacheKey`/`compute_cache_key` in `src/id_cache.rs`). On load, it recomputes the current key and compares it against the stored key; on any mismatch, it silently recomputes and overwrites. `tree_sha` is obtained via `git rev-parse <ref>:knowledge`, and `tool_version` uses the crate version at build time (`CARGO_PKG_VERSION`). However, `canonicalization_rule_version` and `id_index_schema_version` are currently fixed at "1"; the normalization-rule revisions and format revisions that would actually bump these versions have not yet occurred. Virtually including uncommitted working-tree changes in the key via `git hash-object`, and the TTL safety net on the CI shared-storage side, are not implemented (the latter is, in any case, outside the responsibility of the CLI alone). The id itself has also been revised in line with the design policy that "id is path-independent," and currently **uses the `id:` field of feature.yml as the canonical source** (id_cache.rs determines the id by reading the content of feature.yml via `git show`, rather than the directory name). This allows the same Feature to continue being tracked after a Feature directory rename, as long as the `id:` field does not change (minimal rename tracking). The presence of multiple Feature directories sharing the same id is detected as an error. However, a general-purpose, independent id-to-path index layer (e.g., tracking arbitrary id changes not accompanied by a path change) has not been implemented, so this is not a full realization of the target design (Section 3.6).

### 3.4 Finalizing Lineage at Milestone Boundaries

Lineage is finalized not per-commit but only at the time a milestone is finalized (e.g., a release tag). For each id, the "tree as of the previous milestone" and the "tree as of the current milestone" are compared via ID resolution, and if they differ, a `derived_from` relationship is deemed to hold and a `ChangeEvent` is generated (recorded in the implementation as `from_tree_sha`/`to_tree_sha`, Section 3.5).
...
Of these, the items "Simplified from the design" do not affect what the RQ1 evaluation (Chapter 5) primarily requires: "linear version-history tracking at milestone boundaries." Because branch-divergence detection via `git merge-base` is now automatically reflected into the primary lineage of `changes compute` for every merge occurring anywhere within a milestone interval, the precision of version-history tracking can be ensured even in a case study (Section 5.2) of an organization with complex branching practices, without needing to separately invoke the `lineage` command as an auxiliary step.
...
- **Stratum α (shallow change)**: A change within the most recent release. Within the range the control-group workflow can, in principle, handle. It is stated in advance, as an expected result, that the control group will have an advantage in terms of speed.
- **Stratum β (deep change)**: A derivation from several generations back, or a change spanning multiple releases. Because the control-group workflow does not systematically retain derivation relationships across versions, the information needed to answer correctly is itself missing, regardless of proficiency. This is the stratum on which this study's core claim directly bears.
...
3. **Semantic irrelevance**: We confirm whether the changed test file matches the model's structural generation relationship (Section 3.2(A): `FEATURE`/`CONDITION` → `TESTCASE`), and adopt it as a strong candidate only if it matches. A test file that appears with extremely high frequency across the entire commit history (e.g., a smoke test) has low specificity and is down-weighted or excluded.

**Two-stage construction process**: (1) a candidate set is built via mechanical filtering under the criteria above, and (2) a lightweight confirmation (Yes/No) is performed by multiple independent experts (at least two), and inter-rater agreement (e.g., Cohen's kappa) is reported. Candidates on which opinion is split are either excluded from the ground truth or treated as partial credit.
...
Subjects are presented with actual past changes from the target project and asked to identify the set of affected TestCases. The primary metric is accuracy (precision and recall) in Stratum β. Speed and subjects' subjective confidence (e.g., NASA-TLX) are secondary metrics. Sample size targets 15–30 subjects per group to withstand statistical testing, and subjects' years of experience, familiarity with the target project, and proficiency with the current workflow's tools are recorded as covariates. For tasks in the experimental group, the candidate count of `impacted_testcases` (the denominator from the Feature-level conservative candidate extraction of Section 3.5) is also reported alongside precision and recall, so that, if an excessively large candidate set arises in a stratified task, this is visible from the results.
...
- **Construct validity**: The definition of "deep change" is fixed in advance. The breakdown of ground-truth construction method (artifact-based vs. interview-based) is stated explicitly, and where the interview-based proportion is high, a caveat is attached to the results. The co-change noise-removal criteria (thresholds on changed-file count and appearance frequency) depend on the target project's scale and development culture, and would need adjustment before being transplanted directly to another project.
...
- Implementing a general-purpose, independent id-to-path index layer (tracking id changes that do not change the path, etc. — an item that Section 3.3 currently characterizes as staying at "unifying id = the id: field of feature.yml").
...
- An extension that explicitly persists and makes queryable the `derived_from` relationship as a persistent version-history graph (a Version DAG) with version nodes and edges. Currently it is derived anew, per milestone interval, as a `from_tree_sha`/`to_tree_sha` comparison on `ChangeEvent`, and is not held as a persistent graph structure (Section 3.2).
...
This study designed a model that combines Git's content addressing (tree SHA), a non-committed ID-resolution cache, and ancestor search over the commit graph (`git merge-base`) to derive test-knowledge version history (`derived_from`) at milestone boundaries, independent of branching workflow (Chapter 3). Against the structural gap that neither existing TMSs nor naive Git-based workflows treat multi-generational derivation relationships of test knowledge as a first-class query target (Chapter 2, Section 2.4), this model is a design proposal that responds with a milestone-boundary ChangeEvent model built on Git's own object model.
...
**Empirical verification as Future Work**: Verification of RQ1 via a human-subjects experiment is positioned as the direct sequel to this study (Chapter 7). The evaluation plan in Chapter 5 (task stratification, ground-truth construction, consolidation of the control group) has been fixed as pre-registration content ahead of the start of the experiment, and the next stage is to conduct the experiment per this plan and report the results. In addition, empirical verification of the ID-resolution cache's version-revision workflow (Section 3.3) and measured backfill performance on a large-scale repository (Chapter 4) remain as implementation tasks to be pursued independently of evaluating the model itself (the primary-lineage integration of `changes lineage` is already implemented, as described in Section 3.2, "Integration (added 2026-08)," and is therefore excluded from this list).
