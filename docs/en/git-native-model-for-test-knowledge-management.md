...
| Proposed design, not implemented | ADR 0013 replaces the former independent id-to-path-index / alias alternatives with immutable UIDs, limited identity declarations, UID-based relations, migration manifests, and a non-committed derived Registry. Until that schema-version-2 cutover, the current implementation still treats `feature.yml`'s `id:` as identity and changing it severs continuity (decisions/0004). This row describes the current implementation constraint, not a competing future design. |
...
- Implementing and evaluating ADR 0013's schema-version-2 identity model: immutable UIDs for all persistent Knowledge elements, limited identity declarations, UID-based TestCase/Execution/ChangeEvent continuity, legacy migration, crash recovery, and repository-integration behavior. The former independent id-to-path-index and alias approaches are superseded alternatives, not parallel Future Work.
...
- An extension that explicitly persists and makes queryable the `derived_from` relationship as a persistent version-history graph (a Version DAG) with version nodes and edges. This is distinct from ADR 0013's identity-lifecycle causal graph: the latter resolves which logical entity is being compared, while `derived_from` remains a content-version relationship derived anew per milestone interval from `ChangeEvent` tree-SHA comparisons (Section 3.2).
...
However, the immutable UID, identity events, and UID-based tracking of TestCases, Executions, and ChangeEvents are ADR 0013's Proposed design and are not yet implemented in the reference implementation as of this draft. The current implementation can track the same Feature only insofar as `feature.yml`'s `id:` remains unchanged. The usefulness and implementation quality of the UID model therefore remain to be verified.
