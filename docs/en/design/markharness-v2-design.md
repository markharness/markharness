...
`Scenario` can carry the same relation too, as its own `requirement_uids` ([0031](../decisions/0031-scenario-level-requirement-contribution.md)). StrictDoc's Low-Level Requirements are often written at the granularity of one concrete behavior (= one Scenario), and a Feature-level relation alone produces the wrong result whenever one Feature contributes to more than one Requirement: every TestCase under it inherits every one of those Requirements, including ones it has nothing to do with. The generated TestCase's `generated_from.requirement_uids` uses only the Scenario's value when it has one or more entries, falling back to the Feature's otherwise (never a union). `validate` does not detect the two disagreeing. markharness keeps none of StrictDoc's own Requirement hierarchy (HLR/LLR, Parent relations) and treats Requirements as a flat set (P5).
...
| `traceability_index.rs`'s Requirement index and `GeneratedFrom.requirement_ids`/`requirement_uids` | Implemented | Kept |
...
| `plan.rs` (evidence applicability judgment) and `markharness plan` | Removed | [0020](../decisions/0020-execution-status-lightweight-model.md) removes the need for strict matching, and what would remain — reporting whether an `ExecutionBinding` exists — duplicates `coverage` ([0026](../decisions/0026-module-inventory-and-plan-removal.md)). The plan paths in `application.rs` and `presentation.rs` go with it |
| `src/identity/feature_ops.rs`'s `retire_entity`/`restore_entity`/`release_id`/`reissue_entity` and their error types (~524 lines), `event.rs`'s `IdentityMutation::{Retired, Restored, Released, Reissued}`, the corresponding status transitions and `Status::Retired` in `engine.rs`, and the reissue-dependent part of `migration_manifest.rs` | Reduced | [0021](../decisions/0021-identity-retire-simplification.md). The affected files were identified by measurement on 2026-09-11 (the `release` occurrences in `recovery.rs` and `lock.rs` are `IdentityLock` file-lock releases, unrelated to this; `audit.rs` contains none of these terms). [0026](../decisions/0026-module-inventory-and-plan-removal.md) |
...
| `src/canonical.rs` (ImportSourceArg, etc.) | Reduced | Narrowed to serving `markharness import` (native/JUnit) only. The plan-only types (`CanonicalEvidence`, `EvidenceResult`, etc.) are removed along with `plan`. StrictDoc ingestion is redesigned as a future, separate Adapter ([0026](../decisions/0026-module-inventory-and-plan-removal.md)) |
...
| `src/traceability_index.rs` (Requirement index) | Kept | The existing Requirement↔TestCase reverse lookup is used as-is |
...
| `src/milestone.rs`, `src/lineage.rs` | Kept | `src/changes.rs` uses `milestone::verify_audit_matches_tag` (a fail-closed gate) and `lineage::classify` (merge classification) internally; removing them makes `changes compute` fail to compile ([0026](../decisions/0026-module-inventory-and-plan-removal.md)) |
| `src/backfill.rs`, `src/verify.rs` | Kept | Nothing depends on them in reverse, but neither conflicts with [0020](../decisions/0020-execution-status-lightweight-model.md)–[0025](../decisions/0025-v2-forward-compatible-evolution.md), so there is no positive reason to remove them ([0026](../decisions/0026-module-inventory-and-plan-removal.md)) |
| `src/derived_index.rs` and `markharness cache index` | Removed | Its inputs — `plan::BoundVersions` and `execution::read_all_results` — are removed or replaced. The derived cache appears nowhere in the Change Impact / Release Coverage design and has no CLI integration test ([0026](../decisions/0026-module-inventory-and-plan-removal.md)) |
| `src/audit_scope.rs` | Kept | Part of the output contract of `identity migrate --json`, `identity audit --json`, and `changes compute --json` ([0013](../decisions/0013-immutable-identity-model.md) verification rules) |
...
Persistent records carry a `schema_version`. When multiple record types share a storage area or output, a `record_kind` or equivalent identifies the type. `schema_version` is nevertheless **fixed at `1` for every record kind and is never bumped**: it is not a compatibility mechanism for reading older records but a forward contract that keeps a future record kind from being mistaken for this one. If an older form ever needs reading, it is added as a new type rather than a version branch (see the existing-data rule in §9.1). The `record_kind` values are `execution_binding`, `release_scope`, `requirement`, `change_impact`, and `release_coverage`. V2 does not add every possibly useful future field as optional. A full model is introduced with a separate type and storage contract; information absent from V2 remains `unknown` or `legacy`.
...
| AC41 | Feature F `contributes_to` two Requirements R1 and R2; a Scenario S under F sets `contributes_to` to only R1 | The TestCase generated from S has only R1 in `generated_from.requirement_uids` (ADR 0031) |
| AC42 | A Scenario under a Feature carries no `contributes_to` (omitted or empty) | Its generated TestCase inherits the Feature's `requirement_uids` as-is (ADR 0031) |
| AC43 | Run `coverage --requirements R2` (against Feature F from AC41, whose only Scenario is S naming only R1) | S does not appear in `cases`. Even though F `contributes_to` R2, no TestCase under it actually verifies R2, so it is reported as a `feature_has_no_case` gap (ADR 0031) |
