...
Propagating via `io::Error` would add an unreachable error path, which conflicts with the project's design policy (CLAUDE.md) of not adding error handling for scenarios that cannot occur. To keep a future refactor from silently breaking this guarantee, a unit test pinning down that `derive()`'s output is always 36 characters and non-blank regardless of input (including an empty-string input) **is planned for `identity/derived_uid.rs` — not yet implemented; tracked as Step 6 of `checklist-issue-43-typed-identifiers.md`**.
