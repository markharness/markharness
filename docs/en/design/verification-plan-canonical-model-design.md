...
## 7. Typing `bound_versions` (applying [decisions/0017](../decisions/0017-scenario-case-revision-and-execution-evidence.md) to the production path, Issue #43)

[decisions/0017](../decisions/0017-scenario-case-revision-and-execution-evidence.md) §3 requires Case UID and Case revision to be distinct types, but at Step 1a (introducing the types) the production path — `ExecutionEntry`, `PlanEvidence`, `CaseVersion`, `TestCase`, `CaseDefinition`, etc. — still used `String`/`BTreeMap<String, String>`. This section records the design for actually enforcing that contract in the production path's Rust type system.

### 7.1 Structuring `PlanEvidence.bound_versions`, and separating it from `CanonicalEvidence.bound_versions`

The table in Section 1.4 lumped `bound_versions` together as a single Evidence-area field, but tracing the implementation showed two fields of different character sharing the same type (`BTreeMap<String, String>`):

- `plan::PlanEvidence.bound_versions` and the `bound_versions` carried by `derived_index`'s execution index: built from an `execution::ExecutionEntry` with three required fields — `case_uid`/`case_revision`/`target_revision` — and the optional `environment` field included only when it has a value. The field meanings and set are fixed, so there is no need for a map that permits arbitrary keys.
- `canonical::CanonicalEvidence.bound_versions`: holds whatever `markharness import --bind ARTIFACT_ID=VERSION` (an arbitrary number of arbitrary keys) supplies, for external importers. `application::build_verification_plan_value` deliberately excludes evidence from this path from the Plan's evidence candidates (the "Evidence resolution" step in the Section 2.1 diagram only considers `ExecutionEntry`-derived evidence).

Only the former is replaced with `struct BoundVersions { case_uid: CaseUid, case_revision: CaseRevision, target_revision: TargetRevision, environment: Option<Environment> }`, eliminating key typos and omissions at compile time. `CanonicalEvidence.bound_versions` is left as a generic `BTreeMap<String, String>`. The two sharing a name and a type while carrying different contracts is deliberate; unifying them is not a goal.

### 7.2 New typed identifiers

`target_revision` and `environment` are not among the types [decisions/0017](../decisions/0017-scenario-case-revision-and-execution-evidence.md) §3 enumerates, but `execution::record_execution` already had dedicated validation logic rejecting blank strings (`EmptyTargetRevision`, `EmptyEnvironment`). `TargetRevision` and `Environment` are added using `identity::typed_uid`'s existing `define_typed_uid!` macro as-is, aligning this validation with Case UID and the others. Input-boundary validation happens at `record_execution`'s own function boundary (no separate conversion at the CLI layer), and `RecordError::EmptyTargetRevision`/`EmptyEnvironment` are kept for CLI error-message compatibility (wrapping `BlankValueError` internally).

### 7.3 Deliberately out of scope

- Splitting the type of `canonical::CanonicalArtifact.uid` (an `Option<String>` used polymorphically for Feature/Scenario/TestCase): this is a structural change to an existing field, a larger design decision than "propagate to the production path," deferred to a future issue/ADR.
- `generate::GeneratedFrom.feature_uid`/`requirement_uids`: informational only, never used as a matching key, so there is no real risk of confusion to address.
- `identity/migration_manifest.rs` (the legacy `case_id` → `case_uid` Rosetta stone): a separate concern from the evidence/plan matching path, split out as a new issue. Being free to make breaking changes during the prototype phase is not a reason to bundle unrelated concerns into one issue.

### 7.4 Failure policy for reconstructing `CaseUid` from `CanonicalArtifact.uid`

When `application::build_verification_plan_value` assembles `case_versions`, it must reconstruct `CaseVersion.case_uid: CaseUid` from `CanonicalArtifact.uid` (out of scope for type-splitting per §7.3, left as `Option<String>`). This conversion uses `CaseUid::new(value).expect(...)`, not propagation via `io::Error`.

The safety argument is not an indirect "trust something upstream" — it rests on the structure of `identity::derived_uid::case_uid` (`derive()`) itself. `derive()` formats a SHA1 hash with the fixed pattern `format!("{:08x}-{:04x}-{:04x}-{:04x}-{:012x}", ...)`, so it always returns a 36-character, non-blank string regardless of the value of its input (`scenario_uid`). So as long as a value passed to `CaseUid::new(...)` is this function's output, there is no case in which it can structurally fail. This is separate from the `None`-skipping `?` inside the `filter_map` (which excludes an un-migrated Scenario whose `uid` itself is absent) — it is the argument for why the content of a `Some` can never be blank.

Propagating via `io::Error` would add an unreachable error path, which conflicts with the project's design policy (CLAUDE.md) of not adding error handling for scenarios that cannot occur. To keep a future refactor from silently breaking this guarantee, `identity/derived_uid.rs` has a unit test pinning down that `derive()`'s output is always 36 characters and non-blank regardless of input, including an empty-string input (`case_uid_is_well_formed_even_for_a_blank_input`, `case_revision_is_well_formed_even_for_a_blank_input`).
