...
### 11. This ADR's versioning contract does not apply during the prototype (0.x) stage (added 2026-09-01, during the [decisions/0016](./0016-behavior-condition-precondition-step-result-model.md) design discussion)

This project is still at a 0.x, prototype stage of `Cargo.toml`'s version, and the [release-and-license instructions](../../.github/instructions/release-and-license.instructions.md) state that "0.x versions may break compatibility between minor versions" and that "the criteria for 1.0 are tracked in `PROJECT.md` (currently: UC8 importer implementation and schema stabilization)." In other words, the Knowledge schema itself is expected to stabilize on the way to 1.0, and bumping `[knowledge].schema_version` on every prototype-phase design iteration would leave a developer reading this ADR's history after a real release unable to tell which version boundaries mark an actual, deliberated breaking change versus mere prototype churn.

So this ADR's contract (§1-10) only starts to apply once the schema has stabilized (met `PROJECT.md`'s 1.0 criteria); `[knowledge].schema_version` is not bumped for breaking changes made during the prototype phase (including field additions/renames to Behavior/Condition/ExpectedResult). Should a schema-breaking change land after 1.0, once real data exists, it follows this ADR's contract as usual and bumps the version. This judgment applies together with each schema-changing ADR's own confirmation that no real data exists yet, so there is no live risk of misinterpreting a comparison against old-format data at the time that ADR lands.
