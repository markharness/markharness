...
**候補抽出の粒度(2026-08追記、issue #15で製品化)**：`impacted_testcases`の既定動作は、変更が検出されたFeatureに対応する全TestCaseを候補として返す、Feature単位の保守的な候補抽出である(`src/changes.rs`)。`markharness changes compute --granularity <feature|behavior|condition>`(既定`feature`)により、Behavior/Conditionディレクトリ単位のtree SHA比較へ絞り込む方式を利用者が明示的に選択できる。`behavior`/`condition`はFeature単位の変更検出そのもの(`ChangeEvent`をどのFeatureについて1件生成するか、rename追跡、`true_divergences`判定)には影響せず、`impacted_testcases`の絞り込みにのみ作用する。選択した粒度と絞り込みの根拠は、各`ChangeEvent`の`impact_reason`フィールド(`granularity`と`changed_paths`)に記録される。`changed_paths`は、実際にtree SHAが変化し絞り込みの根拠となったBehavior/Conditionのマーカーファイルパス(`behavior.yml`/`condition.yml`)の一覧であり、個々のBehavior/Conditionを解決しない`feature`では空配列になる。

Behavior/Conditionのスキーマ(`feature.yml`/`behavior.yml`/`condition.yml`)には兄弟間の依存関係を表すフィールドが存在せず、本コマンドはそれを検出・推論しない。したがって`behavior`/`condition`は、Feature境界に著者が暗黙に込めた関連性(共有のセットアップ・前提条件等)を意図的に無視した上で、再現率(recall)を精度(precision)と引き換えるオプトインの選択である。この安全性はツール側では保証できないため、既定値は引き続き`feature`のままとし、`behavior`/`condition`を選ぶかどうかの判断はプロジェクトの実情に応じて利用者に委ねる — 依存関係の自動推論を追加する形の精密化は行っていない(第7章 Future Workで触れる粒度診断とは別の設計判断)。
...
- **Gherkin(`.feature`)との連携機能の実装**(UC8の一部、未実装、[decisions/0016](./decisions/0016-behavior-condition-precondition-step-result-model.md)により今後必ず実装する機能として明示)。[0016](./decisions/0016-behavior-condition-precondition-step-result-model.md)が導入した`preconditions`/`steps`/`additional_preconditions`/`additional_steps`/`results`により、素直な`Feature`+`Background`+`Scenario`+`Given`/`When`/`Then`の大部分は表現可能になった。連携は**双方向のラウンドトリップとしてではなく、独立した2つの一方向機能**として設計する方針である。①インポート(Gherkin→markharness、UC8本体): `.feature`ファイルを人間レビューを介してmarkharness YAMLへ変換する。`Scenario Outline`+`Examples`・`Data Table`/`Doc String`・`Rule:`キーワード・タグと`axis`の対応・非正準順序のGiven・`When`を持たないScenarioのように意味モデルに構造的な受け皿がない構文は自動変換せず人間に警告する(常に人間監督下の変換であり、無条件の自動無損失変換は要求しない)。この変換を1回限りの移行として使うか繰り返し変換にかけるかは規定せず利用者の運用に委ねる。トレーサビリティのため、変換で生成した`behavior.yml`/`condition.yml`に元の`.feature`ファイルパスと対象Scenario名を記録する`source`フィールド(トレース専用)を持たせ、繰り返し変換する運用を選んだ場合はこれを既存`uid`維持の照合キーとしても使える。②エクスポート(markharness→Gherkin、都度再生成可能): `TestCase`から`.feature`を生成する、`generated/testcases/*.yml`と同じ生成物パターンのレンダリング機能。詳細は[product-operation.md §4](./product-operation.md)を参照。
- ~~Condition/ExpectedResult差分に基づく候補抽出の精密化~~(2026-08実装、issue #15)。`markharness changes compute --granularity <feature|behavior|condition>`として実装済みで、既定は引き続きFeature単位の保守的な候補抽出のまま、Behavior/Condition単位への絞り込みを利用者がオプトインで選択できる(第3.5節)。ただし依存関係の自動推論は行わないため、絞り込みが安全かどうかの判断自体は引き続き利用者に委ねられる。これによる適合率向上・false negativeリスクの実測は第5章の評価計画で確認する。
...
- **2026-09-01(20)**：前項(19)で「①インポートは1回限りの変換」と記述していたのが不正確だったため訂正。実行頻度・継続的に運用するか1回限りの移行として使うかはこのADR/本稿では規定せず利用者の運用に委ねる、という表現に変更した。あわせて、繰り返し変換にかけられる可能性がある以上`source`フィールド(元の`.feature`ファイルパス+対象Scenario名)は単なるトレース情報だけでなく、再変換時に既存`uid`を維持したまま更新するための照合キーとしても使える、という位置づけを明記した。第7章Future Work・`product-operation.md` UC8補足・decisions/0016の対象外節を同じ内容に統一した。本稿(ja/en)・`product-operation.md`(ja/en)・ADR 0016(ja/en)の記述更新。コードの機能的な変更は伴わない。
- **2026-09-01(19)**：前項(18)の「Gherkin(`.feature`)からのインポータ」を、「双方向のラウンドトリップ」ではなく「独立した2つの一方向機能(①Gherkin→markharnessの1回限りの変換インポート、②markharness→Gherkinの都度再生成可能なエクスポート)」として再整理した。①は1回限りの人間監督下の変換であるため、Scenario Outline/Data Table/Rule/タグ等の意味モデルに受け皿がない構文を無条件に自動変換する必要がなくなり(人間に警告し手動対応させればよい)、変換後は通常のUC1(markharness YAML直接編集)に合流するため`uid`の再生成安定性問題も生じない。②は`generated/testcases/*.yml`と同じ生成物パターンにそのまま乗る。また①の最低限のトレーサビリティとして、変換で生成する`behavior.yml`/`condition.yml`に元の`.feature`ファイルパスと対象Scenario名を記録する`source`フィールド(トレース専用、生成には使わない)を追加する方針を明記した。第7章Future Work・`product-operation.md` UC8補足・[decisions/0016](./decisions/0016-behavior-condition-precondition-step-result-model.md)の対象外節を同じ内容に統一した。本稿(ja/en)・`product-operation.md`(ja/en)・ADR 0016(ja/en)の記述更新。コードの機能的な変更は伴わない。
- **2026-09-01(18)**：[decisions/0016](./decisions/0016-behavior-condition-precondition-step-result-model.md)(0015を置き換え、Behavior/Condition/ExpectedResultへの前提・手順・結果モデル導入)を受け、第7章Future Workへ「Gherkin(`.feature`)からのインポータの実装」を新規項目として追加した。0016により素直なFeature+Background+Scenario+Given/When/Thenの大部分は変換可能になった一方、Scenario Outline+Examples・Data Table/Doc String・Ruleキーワード・タグとaxisの対応・非正準順序のGiven・Whenを持たないScenario(`condition.steps`の`minItems:1`と非両立)・description(Condition/ExpectedResultとも既存の必須フィールドだがGherkin側に対応する自由記述が保証されない)は変換先が未定義、または変換元データが保証されておらず、別ADRでの方針決定が必要である旨を明記した(初稿では「損失なく変換できる」と過大に記述していたレビュー指摘を受け、上記2項目を追加し表現を訂正)。あわせて`product-operation.md`のUC8(既存ツールからインポートする)にGherkin資産を対象へ追加し、同じ未解決項目を補足として追記した。ADR本文(0016)の追加・0015のステータス変更(Superseded)に伴う本稿(ja/en)・`product-operation.md`(ja/en)の記述更新。コードの機能的な変更は伴わない。
- **2026-08-27(17)**：PR #36のレビュー指摘(Spec不一致)に対応。issue #15本文は`impact_reason: {granularity, changed_paths}`という構造(絞り込みの根拠となった具体的なBehavior/Conditionのパスを含む)を明示的に要求していたが、前項(16)の実装では`granularity`スカラーのみを記録し`changed_paths`を欠落させていた。`ChangeEvent`の`granularity`フィールドを`impact_reason`(`granularity`・`changed_paths`)に置き換え、§3.5・1.12節(cli-manual ja/en)の記述・出力例を修正した。コードの機能的な変更(`src/changes.rs`)に伴う本稿(ja/en)の記述更新。
- **2026-08-26(16)**：issue #15への対応として`markharness changes compute --granularity <feature|behavior|condition>`(既定`feature`)を実装した。§3.5の「候補抽出の粒度」段落を、第7章Future Workで据え置いていた「Condition/ExpectedResult差分に基づく候補抽出の精密化」がオプトインのCLIオプションとして実装済みであることを反映する記述に更新し、既定値はFeature単位のまま変わらない旨・依存関係の自動推論は行わない旨(false negativeリスクは利用者判断に委ねる設計)を明記した。§7 Future Workの該当項目に取り消し線を付し実装済みである旨を追記した(項目自体は削除せず、残る限界として「絞り込みが安全かどうかの判断は依然利用者に委ねられる」ことを明記)。§6 Threats to Validityの「粒度依存性」(Feature境界自体の設計品質、issue #15とは独立の論点)は変更していない。コードの機能的な変更(`src/changes.rs`・`src/id_cache.rs`・`src/cli.rs`)に伴う本稿(ja/en)の記述更新。
