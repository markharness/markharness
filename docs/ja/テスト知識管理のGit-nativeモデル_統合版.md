...
**位置づけ**：本稿は [ADR 0017](./decisions/0017-scenario-case-revision-and-execution-evidence.md) に基づく設計提案と評価計画である。本文・図・例はすべてこの採用モデルを記述する。設計は合意済みだが、新モデルの実装・インポート試験・性能測定・被験者実験は未完了であり、有効性を実証した論文ではない。未決定事項は第4章と第7章に示す。旧モデルの説明は付録Aの変更経緯に限定する。

## 1. Introduction

### 1.1 動機と対象

テスト定義を変更したとき、実行者は「何が変わったか」「既存の実行結果はどの定義に対するものか」「対象リリースの確認に使える証跡はあるか」を判断する必要がある。表示名、保存場所、手順の内容、対象ビルドを一つの識別子や版にまとめると、名称変更による履歴の分断や、異なる定義・対象の結果の取り違えにつながる。

本モデルはテスト知識から具体的なケースを生成し、その同一性、検証内容の版、実行対象、証跡の適用可能性を分離する。markharness の責務はテストケースの管理、版を軸にした変更検知、実行証跡の関連付けである。実行コードは別途作成・管理し、実行と最終結果の判定は外部ツールまたは手動実行の担当者に委ねる。
...
点線はコード生成を意味しない。ケースとの対応付けは、実行コードがその内容を正しく実装していることの証明ではない。
...
4. 固定した snapshot 間の変更情報と、ケース版・対象・環境を伴う証跡を結び付ける。
...
履歴比較と証跡の適用判定に必要な内部データを Git で保存し、専用サーバーや Git 外の正準データベースを必須にしない。identity の宣言、取り込み済み Knowledge、必要な対応情報、固定された実効定義、採用する実行記録は、この再現可能性を支える入力である。キャッシュと検索インデックスは派生物であり、削除後に再構築できなければならない。
...
Doorstop は item の fingerprint と reviewed fingerprint を比較し、リンク先についてもレビュー時の fingerprint を使って変更を検出する。したがって、内容に由来する版や関連先の変更検出自体を本モデル固有の発明とは位置づけない。本モデルでは、共通手順を展開したケース定義と、実行したケース版・対象・環境との関係を評価対象にする。[Doorstop Item Reference](https://doorstop.readthedocs.io/en/v2.0/reference/item/)
...
CASE_DEFINITION は Case UID と Case revision の組で識別される固定定義であり、TESTCASE の新しい論理 identity ではない。対象・環境の形式、計画ごとの必須項目、空 Phase 等の検証規則は未決定である。ChangeEvent、snapshot、CanonicalSnapshot の関係は第3.5節で示し、未決定の保存形式を ER 図に捏造しない。
...
1 Scenario = 1 TestCase を正式な契約とする。Case UID は Scenario UID から型を区別して決定的に導出し、生成のたびに乱数で発行しない。表示 ID、FeatureUid、ScenarioUid、CaseUid、ExecutionUid、版参照は区別し、表示 ID を証跡照合の代替キーにしない。
...
編集者または明示的な外部対応情報が継続性を指定する。本文の類似から推測しない。分割・統合の由来は記録するが、新しいケースへ過去の合格を引き継がない。由来の記録形式は未決定である。

[ADR 0013](./decisions/0013-immutable-identity-model.md) の identity 宣言、決定性、回復に関する安全性は維持する。新しいエンティティ構成での発行・退役・復元等の具体的な適用は実装設計で整理する。
...
| 名前、説明、実装メモ、出典 | 変更しない |
| 分類タグ、要件との関連 | 変更しない。計画・関連の変更として扱う |
| 所属移動 | 実効内容が変われば変更 |
| 対象ビルド、実行環境 | ケース版と分離して証跡で照合 |
...
### 3.5 snapshot 比較、ChangeEvent、計画

マイルストーンタグ等で指定する固定 snapshot 間の比較を、版を軸にした追跡の基礎にする。比較時に必要な Knowledge、identity 宣言、外部対応情報を対象 snapshot に固定する。証跡判定も選択した記録を入力とし、外部サービスの現在値やキャッシュの有無によって過去の結果を変えない。
...
flowchart TB
  ...
  C --> P
  E["保存された実行証跡"] --> S["明示的に証跡を選択"]
  S --> P
  P --> V["結果・適用可能性・証跡の有無"]
  B -. "投影" .-> CS["CanonicalSnapshot\n形式は未決定"]
  C -. "投影" .-> CS
  E -. "投影" .-> CS
```

ChangeEvent は変更情報、VerificationPlan は対象と採用証跡の指定を担う。CanonicalSnapshot は内部知識・生成物・証跡を連携に向けて投影するもので、手順の第二の編集正本にはしない。図の矢印は論理的な入力関係であり、未決定の API や自動候補選択アルゴリズムを確定するものではない。
...
実行結果（pass/fail/skip）、適用可能性、証跡の有無を別概念とする。計画は採用する実行記録を明示し、Case UID、Case revision、対象ビルド、要求環境が適合するときだけ対象計画の証跡として使う。

| 記録と対象計画 | 判定の意味 |
|---|---|
| 選択した pass が全条件に適合 | 対象ケースの合格証跡として使用可能 |
| 選択した fail が全条件に適合 | 対象ケースの失敗として保持 |
| 記録は pass だがケース版またはビルドが違う | 過去の pass は残すが、この計画の合格には使わない |
| 対象・環境が不明、参照が未解決、記録が破損 | 合格を満たす証拠として使わない |
| skip、または記録がない | 合格と扱わない |

表は個別ケースへの適用条件を示す。計画全体の集計・状態名・終了コードの最終仕様は未決定である。「実行済み」と「合格済み」を混同せず、過去の fail を stale で置き換えない。独立した結果を時刻だけで上書き・優先しない。訂正方法と競合時の操作は実装設計で定める。

実行・リトライ・集約・最終結果の決定は外部側が担当する。markharness は Run/Attempt モデルや flaky 判定を持たず、再試行後の成功を独自に不合格へ変更しない。手動記録にも同じ参照・適用条件を用いる。

### 3.7 保存単位と再構築

実行記録はケースごとに論理的に独立させ、外部レポートを個別記録へ分解して取り込む。元レポートへの参照を残し、重複取り込みを防ぐ。物理的に1ファイルへ何件保存するか、重複キー、原子的な取り込み、破損時の保全方法は未決定である。

検索インデックスやキャッシュは正本から再構築する。保存する実効定義は不変の証跡であり、再生成対象の最新表示と区別する。具体的なディレクトリ配置は確定していないため、本稿では既存 CLI の保存パスを新モデルの契約として示さない。

### 3.8 設計の到達点と未確定の契約

確定しているのは、所属と関連の分離、Scenario と Phase の所有、明示的な共通手順参照、ケース同一性、revision の対象項目、証跡の責務、外部編集・実行との分担である。第3章の図はこれらを表す概念図である。

未確定のものは、正規化と型の詳細、空配列等の検証、ChangeEvent / 計画 / CanonicalSnapshot の厳密な形式、対象・環境の表現、保存・訂正・競合処理、インポートの更新規則である。これらを埋めずに実装可能な全仕様が完成したとは扱わない。

## 4. Implementation Plan

### 4.1 実装状況と分割

本稿の新モデルは未実装である。既存 CLI のテスト成功や過去の実装報告を本モデルの動作保証として用いない。[Issue #40](https://github.com/markharness/markharness/issues/40) で追跡する設計・実装を次の単位に分ける。
...
4. 実行記録と計画への明示的な証跡選択を統一する。
5. 実際の外部入力を用いて継続インポートを実装・検証する。
...
Playwright コードは別途管理し、実行側が対応する Case UID / Case revision を明示する。結果取り込み時に最新版を補完すると実行していない定義への証跡になり得るため、補完しない。未解決・曖昧な結果を検証済み証跡として採用しない。
...
プロトタイプ中は形式バージョンを更新せず新モデルに置き換える。旧形式 reader、互換レイヤー、移行・自動変換、専用の旧形式識別機構は作らず、新スキーマで入力を検証する。Case revision はこれとは別の内容追跡の概念である。
...
- 分割・統合時の新規 identity と、合格証跡を引き継がないこと。
- 旧版・別ビルド・環境不明・skip・欠落・破損を合格にしないこと。
- 重複取り込み・並行記録・中断後の保全と回復。
- 固定入力からの再計算、キャッシュ有無の同値性、実行時の版を最新版で置換しないこと。
...
ケース版・対象・環境の照合精度は機能検証で確認する。RQ1 の人の判断支援効果と、保存容量・生成時間・検索時間の性能測定は別に評価する。本稿に実測値はない。

## 6. Threats to Validity

- **構成概念**：定義の差分は意味上の影響全体ではない。手順を更新していない要件変更や、外部コードとの不一致を見落とす可能性がある。
- **内的妥当性**：UI、学習時間、明示的な対応付けや証跡選択の追加作業が結果へ影響する。
- **入力の信頼性**：実行側が誤った Case revision や対象を申告すると、照合だけでは誤申告を証明できない。取り込みはテスト実装の意味的正しさを保証しない。
- **外的妥当性**：単一組織・特定 Gherkin 記述規約・特定ランナーでの結果を一般化しない。
- **正規化と保存**：意味のある差分の除外、入力依存の欠落、固定定義の増加、外部レポートの消失を検証課題とする。
- **未実装**：概念上の整合性は、並行書き込みやクラッシュ時の正確性・実務上の性能の証明ではない。

## 7. Future Work

- ADR 0017 の未決定項目を具体化し、実装と機能検証を完了する。
- ChangeEvent とケース版差分・要件関連変更の接続を定め、複数世代の照会を検証する。
- Gherkin の継続取り込みと外部結果の対応を、rename・削除・Examples の変更を含む実データで検証する。
...
- 正式リリース時の形式バージョン運用をスキーマ安定後に確定する。

自動実行エンジン、Playwright コード生成、リトライの独自判定、全外部形式の無損失な双方向同期は、今回の採用設計の対象外である。

## 8. Conclusion

本稿は、Scenario を具体ケースの単位とし、不変の Case UID、実効内容に由来する Case revision、Git に固定した定義、対象・環境と結び付いた実行証跡を分離するモデルを提案した。Gherkin と Playwright をそれぞれ編集・実行の担当として維持し、markharness はケース管理と変更検知、証跡の適用確認を担当する。

採用した設計は ADR 0017 に記録されている。実装と評価は未完了であり、正答率・所要時間の改善、完全なインポート適合性、性能上の優位性は結論できない。未決定の契約を具体化し、第4章の機能検証と第5章の評価計画を実施することが次の課題である。
