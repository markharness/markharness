...
`source_key`にはStrictDoc側のUID文字列をそのまま保存する。大文字・小文字を含め、markharness側での自動変換・強制正規化は行わない。文字集合の制限も設けない(`id`の`is_valid_slug`制約は適用しない)。原表記を保持することが、StrictDoc側との目視照合・grep等の手動一致確認を可能にする前提だからである。
