...
記録はケースごとに独立させる。外部レポートは個別記録へ分解し、元レポートへの参照を残して重複取り込みを防ぐ。物理的な1ファイル当たりの件数は想定規模を踏まえて決め、1記録1ファイルは本 ADR では確定しない。
...
対応情報の正本は一か所にする。外部側で Scenario UID 等の対応を管理しているなら、それを受け取り検証する。外部側に対応情報がない場合は Git 管理の明示的な対応表で補う。両側に同じ対応を二重管理しない。

markharness のインポート処理は、対応を検証済みの内部参照へ解決する。名称の類似などによる推測照合は行わない。外部ツールに markharness の UID 発行を要求しない。未解決・曖昧な対応を有効な証跡として登録しない。

特定の外部ツール(構文・実行フレームワーク)との統合方式は本 ADR の対象外とする。必要になった時点で、その対話・要件を根拠とする個別の ADR・Issue として扱う。
また、共通モデルは ADR 0017、Gherkin／ Playwright は想定ユースケースで、統合契約は別途決定
...
- [0015](0015-behavior-step-model.md) / [0016](0016-behavior-condition-precondition-step-result-model.md)：Condition / ExpectedResult の分離、ファイル名順、自動で合成する準備操作を置き換える。任意の共有 Step 台帳は採用しない。0016 の人が確認する一方向インポートの方針は維持する。

## 代替案と帰結

論文との関係は[統合版第3.8節](../テスト知識管理のGit-nativeモデル_統合版.md#38-設計の到達点と未確定の契約)を参照する。従来実装の動作確認は本 ADR の実装検証ではない。Case revision の変化と、要件・実装変更による意味上の再検討範囲は同義ではなく、その候補抽出規則は未決定である。外部を編集の正本とする場合も、内部の過去比較に必要な取り込み済み入力・対応情報を Git の snapshot に固定する必要がある。
...
参照解決、ケース版計算、固定定義の保存は増える。一方、Phase の identity 管理、実行順のファイル名管理、外部との二重編集を減らす。Phase 単独の履歴や元文書の完全な往復再現は保証しない。性能上の優位性は未測定である。

## 決定が満たすべき振る舞い

必須の検証例は、rename と再取り込みでの UID 維持、操作・順序・共通手順変更の版更新、説明だけの変更での版維持、分割・統合時の新規 identity、別版・別ビルド・環境不明・skip を合格に使わないこと、重複取り込みと並行記録、実行時の版を最新値で置換しないことである。

原子的な取り込み・破損保全、重複キー、訂正、時刻の比較、対象・環境の型、版の正規化、参照欠落・空 Phase の検証、各派生モデルの厳密な形式は実装前に決定する。本 ADR だけをもって実装可能な全スキーマが確定したとは扱わない。実装順序・タスク分割は、着手時に`checklist-<task>.md`として別途管理する([checklist-workflow](../../../.github/instructions/checklist-workflow.instructions.md))。
